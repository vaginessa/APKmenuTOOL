...
class ApkDetect:
    ...
    def getXmlInfo(self):
        ...
        if 'Windows' in platform.system():
            xml_cmd = "%s%saapt.exe d xmltree \"%s\" AndroidManifest.xml "%(self.toolsPath,os.sep, self.apk_path)

        if 'Linux'in platform.system():
            xml_cmd = "%s%saapt d xmltree %s AndroidManifest.xml "%(self.toolsPath,os.sep, self.apk_path)
    ...
    def getApkInfo(self):
        ...
        aaptlines=[]
        if 'Windows' in platform.system():
            apkinfo_cmd = "%s%saapt.exe  dump badging \"%s\" "%(self.toolsPath, os.sep, self.apk_path)

        if 'Linux'in platform.system():
            apkinfo_cmd = "%s%saapt  dump badging \"%s\" "%(self.toolsPath, os.sep, self.apk_path)
    ...
    def checkRight(self):
        ...
        soList={}
        for name in namelist:
           if name.split('/')[0]=='lib' and name.find(".so")!=-1:
            ...
            if fileTree:
                ...
                for smallFile in soList.keys():
                    for sss in soList.keys():
                        ret=operator.eq(soList[smallFile],soList[sss])
                        if ret!=1:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        ...
        dit = {'apkPath':'','toolsPath':''}
        dit['apkPath'] = sys.argv[1]
        dit['toolsPath'] = sys.argv[2]
        flag = sys.argv[3]#
        ad = ApkDetect(dit)
        if flag == '1':  #表示获取图标
            ad.saveIcon()
...
#if __name__ == "__main__":
#     #初始化地址
#     dit = {'apkPath':'D:\\Android\\MyApplication2\\app\\testctf2.apk','toolsPath':'D:\\apktools\\tools'}
#     ad = ApkDetect(dit)
#   
#     dit_result = ad.getFileTree()
