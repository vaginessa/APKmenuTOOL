...
class Adb(object):
    ...
    def getSerialId(self, host):
        ...
        try:
            m = re.search(pattern, result)
            if m:
                self.phoneInfoDict['6_serialId'] = "序列号：" + result
                # print result
                return
        except Exception as  e:
    ...
    def getImei(self, host):
        ...
        try:
            m = re.search(pattern, result)
            if m:
                self.phoneInfoDict['7_imei'] = "IMEI：" + str(m.group(1))
                # print str(m.group(1))
                return
        except Exception as  e:
    ...
    def getMac(self, host):
        ...
        try:
            m = re.search(pattern, result)
            if m:
                self.phoneInfoDict['5_mac'] = "MAC：" + result
                # print result
                return
        except Exception as  e:
    ...
    def getDensity(self, host):
        ...
        try:
            m = re.search(pattern, result)
            if m:
                self.phoneInfoDict['9_density'] = "手机密度：" + str(m.group(1))
                # print str(m.group(1))
        except Exception as  e:
    ...
    def getMemory(self, host):
        ...
        try:
            ...
            if m:
                ...
                free = int(free / 1024)
                self.phoneInfoDict['a_memory'] = "内存可用占比：" + str(availableMegs) + '%' + '(' + str(free) + 'M/'+ str(total) + 'M)'
        ...
        if result.find('No such file or directory') == -1:
            ...
            try:
                ...
                if m:
                    ...
                    self.phoneInfoDict['b_storage'] = "data目录可用占比：" + str(availbeStore) + '%'+ '(' + str(free) + units+ '/'+ str(total) + units+ ')'
        ...
        if result.find('No such file or directory') == -1:
            try:
                ...
                if m:
                    ...
                    availbeStore = free/total
                    availbeStore = int(availbeStore * 100)
                    self.phoneInfoDict['c_sdcard'] = "内部SD卡剩余占比：" + str(availbeStore) + '%'+ '(' + str(free) + units+ '/'+ str(total) + units+ ')'
                    # print str(availbeStore) + '%'
                    return
            except Exception as  e:
        ...
        if result.find('No such file or directory') == -1:
            try:
                ...
                if m:
                    ...
                    availbeStore = free/total
                    availbeStore = int(availbeStore * 100)
                    self.phoneInfoDict['c_sdcard'] = "外部SD卡剩余占比：" + str(availbeStore) + '%'+ '(' + str(free) + units+ '/'+ str(total) + units+ ')'
                    # print str(availbeStore) + '%'
                    return
            except Exception as  e:
                self.phoneInfoDict['c_sdcard'] = ''
...
if __name__ == "__main__":
   if len(sys.argv) != 4:
       ...
       dits['adb_path'] = sys.argv[1]
       dits['input_para'] = sys.argv[2]
       flag = sys.argv[3]
       adb = Adb(dits)#
       if flag == 'screenCap':
           adb.getScreenCap()
...
#if __name__ == "__main__":
#     adb_path = 'adb'
#     input_para = 'D:\\Android\\android1\\'
#     # input_para = 'net.ting.sliding'
#     dits = {'adb_path':'','input_para':''}
#     dits['adb_path'] = adb_path
#     dits['input_para'] = input_para#'I:\\dddd\\03-17-46-09.apk'
#     flag = 'screenCap'#
#     #print (dits)
#     adb = Adb(dits)
#     adb.getScreenCap()